...
- `js/main.js` … ギャラリー切替・テーマ変更などのロジック  
- （必要に応じて）追加ファイル・フォルダーは自由に作成可  
  - 例: `audio/`, `report/`, `docs/` など
...
### 11-4. アセット操作ポリシー
1. **画像・音声ファイルの変更・最適化は行わない**  
   - 誤って画質を落とす・メタデータを削除しないように。  
...
### 11-7. ログ & 監査
- すべての Danger-mode 実行ログを `logs/danger-<YYYYMMDD-HHmm>.txt` に保存。  
